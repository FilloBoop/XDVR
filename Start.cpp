...
void Start :: vdomanda(){
	...
	}while(verificacorretta(d,risp) && (l.get_domanda_corrente()<15 || (l.get_domanda_corrente() == 15 && l.getSaltaDomanda())));
	a.reset();
}


void Start :: salvalapartita(){
	char c;
	string s1;
	...
	if (c=='y'){
		...
		out<<s1<<" "<<l.get_domanda_corrente()<<" "<<a.getausati()<<endl;
		out.close();
	}
}

void Start :: vmontepremi(){
	system("cls");
	string s1;
	int v[15] = {50,100,200,300,500,1000,2000,4000,8000,16000,32000,64000,125000,500000,1000000};
	for (int i=15; i>0; i--){
		if (i == l.domanda_corrente)
			cout<<"- "<<i<<") "<<v[i-1]<<endl;
		else
			cout<<i<<") "<<v[i-1]<<endl;
	}
...
bool Start :: verificacorretta(Domanda d, int risp){
	if (d.getCorretta() == risp) return true;
	return false;
}

void Start :: vaiuto(Domanda d){
	int x;
